...
GO
/****** Object:  Trigger [dbo].[SalesOrderDetail_Test_Audit]    Script Date: 3/9/2021 11:27:52 AM ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
ALTER TRIGGER [dbo].[SalesOrderDetail_Test_Audit] ON [dbo].[SalesOrderDetail_Test] FOR INSERT, UPDATE, DELETE
AS

DECLARE 
	@AuditBatchId INT,
	@umTAuditColumns umTAuditColumns,
	@Type CHAR(1),
...
INSERT uManageDBLogs.dbo.tbl_AuditBatch([Type], TableName, AppUserName, AppName, HostName, EventInfo, AuditDate)
		VALUES(@Type, 'SalesOrderDetail_Test', @AppUserName, @AppName, @HostName, @EventInfo, @AuditDate)
		SET @AuditBatchId = @@IDENTITY

BEGIN   ----   SELECT DATA FOR #TMP_COLUMNS WHERE FROM COLUMNSUPDATED DEFINE INPUT PARAMETERS 
		
	INSERT INTO @umTAuditColumns (ColumnId,AuditBatchId,ColumnName,ColumnDataType,isUpdated,isPK,isSkip)
		SELECT
		...
		left join INFORMATION_SCHEMA.KEY_COLUMN_USAGE AS kcu ON kcu.TABLE_NAME = tcPK.TABLE_NAME
			...
			WHERE c.TABLE_NAME= 'SalesOrderDetail_Test'
END
...
BEGIN   ----   PREPARE DATA FOR SQL @SQL
	...
	SET @PKname = (SELECT ColumnName FROM @umTAuditColumns
		WHERE isPK = 1)

	IF @PKname is null 
	BEGIN 
			RAISERROR('no PK on table %s', 16, -1, 'SalesOrderDetail_Test')
		RETURN
	END 
